		    ...
		    local pos = GetEntityCoords(GetPlayerPed(-1))
			 ...
			 if herSpeedKm > minSpeed then			 
			    ...
			    if herSpeedKm < maxSpeed then 
				  ...
				  info = string.format("~b~Placa:~w~ %s ~n~~y~Km/h: ~g~%s~n~~y~Mph: ~g~%s",plate,math.ceil(herSpeedKm),math.ceil(herSpeedMph) )
				 else
				  
				  info = string.format("~b~Placa:~w~ %s ~n~~y~Km/h: ~r~%s~n~~y~Mph: ~r~%s",plate,math.ceil(herSpeedKm),math.ceil(herSpeedMph) )
